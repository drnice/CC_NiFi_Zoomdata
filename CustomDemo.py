...
for x in range(0, int(num_of_trans)):
    ...
    #print("transaction_comma" + transaction_comma)
    splitstring=transaction_comma.split(',')
                                                                                                                                                    #"zip_code","latitude","longitude","city","state","county"
    transaction={x :{'type':'Feature', 'properties': {'date':int(time.time()), 'fname':splitstring[0], 'lname':splitstring[1], 'cc':splitstring[2], 'zip':splitstring[3], 'latitude':splitstring[4], 'longitude':splitstring[5], 'city':splitstring[6], 'state':splitstring[7], 'county':splitstring[8], 'cost':random.randint(1,10000)}}}
    if int(num_before_fraud)==x:
        transaction={'type':'Feature', 'properties': {'fraud': True, 'date':int(time.time()), 'fname':splitstring[0], 'lname':splitstring[1], 'cc':splitstring[2], 'zip':splitstring[3], 'latitude':splitstring[4], 'longitude':splitstring[5], 'city':splitstring[6], 'state':splitstring[7], 'county':splitstring[8], 'cost':random.randint(1,10000)}}
    else:
        transaction={'type':'Feature', 'properties': {'fraud': False, 'date':int(time.time()), 'fname':splitstring[0], 'lname':splitstring[1], 'cc':splitstring[2], 'zip':splitstring[3], 'latitude':splitstring[4], 'longitude':splitstring[5], 'city':splitstring[6], 'state':splitstring[7], 'county':splitstring[8], 'cost':random.randint(1,10000)}}
